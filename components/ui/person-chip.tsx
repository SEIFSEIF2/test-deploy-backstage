...
> = {
  sm: {
    box: 'h-5 w-5 text-[10px]',
    initials: 'text-[10px]',
    name: 'text-[13px]',
    gap: 'gap-2'
  },
  md: {
    box: 'h-6 w-6 text-[11px]',
    initials: 'text-[11px]',
    name: 'text-[13px]',
    gap: 'gap-2'
  },
  lg: {
    box: 'h-9 w-9 text-[14px]',
    initials: 'text-sm',
...
export function PersonChip({
  ...
  return (
    <span className={cn('inline-flex min-w-0 items-center', dims.gap)}>
      <span
        className={cn(
          'bg-muted text-muted-foreground relative inline-flex shrink-0 items-center justify-center rounded-md font-medium tracking-wide',
          dims.box
        )}
        aria-hidden
      >
        ...
        {self && (
          <span
            className="bg-accent ring-card absolute -top-0.5 -right-0.5 h-2 w-2 rounded-full ring-2"
            aria-hidden
          />
        )}
      </span>
